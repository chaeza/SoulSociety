...
public class SwordRain : MonoBehaviour
{
    ...
    private void Start()
    {
        myrect = transform.Find("SkillRange").gameObject.GetComponent<RectTransform>();
    }
    ...
    public void SkillFire()
    {
        if (skillCool == false)
        {
            if (skillClick == false)
                {
                    transform.Find("SkillRange").gameObject.SetActive(true);
                    myrect.sizeDelta = new Vector2(skillRange, skillRange); 
