...
public class Disintergrate : MonoBehaviourPun, ItemMethod//아이템 인터페이스 상속
{
    [SerializeField]
    int itemNum = 0;
    AudioSource sound;
    ...
    public void ItemSkill()//이 아이템이 가지고 있는 아이템 스킬 사용
    {
        ...
        GameObject a = PhotonNetwork.Instantiate("Disintergrate", transform.position, Quaternion.identity);//이펙트를 포톤 인스턴스를 합니다.
        a.AddComponent<MyPosition>();
        a.SendMessage("MyPos", gameObject.transform, SendMessageOptions.DontRequireReceiver);
        a.SendMessage("YPos", 1f, SendMessageOptions.DontRequireReceiver);

        gameObject.GetPhotonView().RPC("Disintergrate", RpcTarget.All, 5f);
        ...
        Destroy(GetComponent<Disintergrate>());//해당 아이템을 사용후 컴포넌트 삭제
    }
}
